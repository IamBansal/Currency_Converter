...
import com.example.currencyconveter.data.models.ConversionRates
    ...
    fun convert(
        ...
        viewModelScope.launch(dispatchers.io){
            ...
            when(val rateResponse = repository.getRates(fromCurrency)){
                is Resource.Error -> {
                    _conversion.value = CurrencyEvent.Failure(rateResponse.message!!)
                }
                is Resource.Success -> {
                    val rates = rateResponse.data!!.conversion_rates
                    try {
                        ...
                        Log.d("MainViewModel", "Error is ${e.message}")
                    }
                }
            }
        }
